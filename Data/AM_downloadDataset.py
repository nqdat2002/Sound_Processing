...
def save_json(searchTerms, birdName, country):
    ...
    path = "./xeno-canto-dataset/" + birdName.replace(':', '') + "/" + country
...
def download(searchTerms, birdName, country):
    ...
    # debug
    # print(len(fileaddress))
    # print(fileaddress)
    if numfiles > 100: numfiles = 50
    for i in range(0, numfiles):
        print("Saving file ", i + 1, "/", numfiles,
              ": ./xeno-canto-dataset/" + birdName.replace(':', '') + filenamesID[
                  i] + ".mp3")

        # Error base on https is false.
...
def main(argv):
    ...
    #     for bird in birds:
    #         download(bird + ' cnt:' + country + ' type:song', bird.replace(' ', ''), country)    countries = ['Poland', 'Germany', 'Slovakia', 'Czech', 'Lithuania']
